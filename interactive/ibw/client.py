...
class IBClient():
    ...
    def _set_server(self) -> bool:
        ...
        if server_account_content is not None and 'set' in server_update_content.keys() and server_update_content['set'] == True:
            print(success)
            return True
        elif ('error' in server_update_content.keys()) and (server_update_content['error'] == 'Account already set'):
            print(success)
            return True
        else:
            print(failure)
        ...
        while (max_retries > 4 or self.authenticated == False):
            ...
            if 'statusCode' in auth_response.keys() and auth_response['statusCode'] == 401:
                ...
                try:
                    ...
                    if 'news' in news_resp:
                        self.authenticated = True

                        #  Log the response.
                        logging.debug('Had to do News Update Response: {auth_resp}'.format(
                            auth_resp=news_resp
                        )
                        )
                        break
    ...
    def close_session(self) -> None:
        ...
        if self._operating_system == 'win32':
            ...
            return_code = os.kill(self.server_process, 9)
    ...
    def _make_request(self, endpoint: str, req_type: str, params: Dict = None) -> Dict:
        ...
        elif url == 'https://localhost:5000/v1/portal/iserver/account':
